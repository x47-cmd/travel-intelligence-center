/* =========================================================
   ...
   Trips Performance Adapter V1.0.0

   File Path:
   js/extensions/performance-integration/trips-performance-adapter.js

   Purpose:
   - Optimizes the Trips page without modifying the frozen legacy file.
   - Applies safe content-visibility to heavy trip sections.
   - Lazily activates expensive page regions.
   - Defers image decoding and non-critical work.
   - Preserves scroll position between route transitions.
   ...
   3) js/extensions/performance-integration/trips-performance-adapter.js
   ========================================================= */

(function tripsPerformanceAdapterBootstrap(global) {
  "use strict";

  if (!global || global.TravelTripsPerformanceAdapter?.version) {
    return;
  }

  const VERSION = "1.0.0";
  const NAME = "TravelTripsPerformanceAdapter";

  const DEFAULT_CONFIG = Object.freeze({
    debug: false,
    route: "trips",
    refreshDebounceMs: 48,
    idleTimeoutMs: 1200,
    minimumVirtualizedItems: 6,
    sectionSelectors: [
      "[data-trips-section]",
      ".trips-section",
      ".trip-section",
  ...
  function applySectionContainment(section) {
    ...
    section.style.setProperty(
      ...
      section.dataset.intrinsicSize || "1px 720px"
    );
  ...
  function applyItemContainment(item) {
    ...
    item.style.setProperty(
      "contain-intrinsic-size",
      item.dataset.intrinsicSize || "1px 220px"
    );
  ...
  function bindMutationObserver() {
    ...
    state.observer = new MutationObserver((mutations) => {
      ...
      if (hasRelevantChanges) {
        scheduleRefresh("trips-dom-changed");
      }
    });
  ...
  function activate(context = {}) {
    ...
    if (!state.pageRoot) {
      warn("Trips page root was not found.");
      return null;
    }
    ...
    try {
      getPerformanceCore()?.emit?.("trips-performance-activated", {
        route: state.config.route,
        result
      });
    } catch (_) {
