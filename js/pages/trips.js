...
(function tripsPageFactory(window, document) {
  ...
  /* =========================================================
     ...
     UI Disabled / Storage Compatibility Edition

     Purpose:
     - Removes "الرحلات المخطط لها" from Trips UI.
     - Stops rendering, styles, listeners and UI calculations.
     - Keeps Store plannedTrips data untouched.
     - Keeps compatibility function names to prevent errors elsewhere.
  ========================================================= */
  ...
  const removePlannedTripsUI = () => {
    document
      .getElementById(PLANNED_STYLE_ID)
      ?.remove();
    ...
    plannedRuntime.pendingActionIds.clear();
   ...
   * plannedTrips remain inside Store, but Trips page no longer
   * reads, renders, edits, promotes or deletes them from its UI.
   */

  const getPlannedTrips = () => [];

  const findPlannedTrip = () => null;
  ...
  const ensurePlannedTripsStyles = () => {
    removePlannedTripsUI();
    return false;
  };

  const renderPlannedTripCard = () => "";
  ...
  const promotePlannedTrip =
    async () => false;

  const deletePlannedTrip =
    async () => false;
  ...
  const bindPlannedTripsEvents = () => {
    removePlannedTripsUI();
    state.plannedTripsClickBound = false;
    return false;
  };
